									...
									<tr>
										<td data-toggle="modal" data-target="#popuppedido_<?=$ido_DoItem;?>">
											...
											if (!empty($img_item) && $img_item != "null" && file_exists("uploads/{$img_item}") && !is_dir("uploads/{$img_item}")):										
												if($detect->isMobile()): 
													echo "<figure class=\"thumb_menu_list\"><img src=\"{$site}uploads/{$img_item}\" /></figure>";
										...
										<td data-toggle="modal" data-target="#popuppedido_<?=$ido_DoItem;?>" id="precodoitem">
											<strong>R$ <?php
											$lerbanco->ExeRead("ws_relacao_tamanho", "WHERE id_user = :useriid AND id_item = :idiitem", "useriid={$getu}&idiitem={$ido_DoItem}");
											if(!$lerbanco->getResult()):
												echo Check::real($preco_item);
											else:
												echo "<span style='text-decoration: line-through;'>".Check::real($preco_item)."<span>";
											endif;
										...
										<form id="addItemPost_<?=$ido_DoItem;?>" name="addItemPost_<?=$ido_DoItem;?>" method="post">
											<td class="options">
												<div class="dropdown dropdown-options">
													...
													if($detect->isMobile()): 
														?>
														<a style="cursor: pointer;" data-toggle="modal" data-target="#popuppedido_<?=$ido_DoItem;?>"  aria-expanded="true"><i style="font-size: 13px;background-color: #34af23;color:#ffffff;padding-left:5px;padding-top: 5px; padding-right: 7px;padding-bottom: 5px; border-radius: 4px;" class="fa fa-cart-plus" aria-hidden="true"></i></a>
														<?php

													else:
														?>
														<a style="cursor: pointer;" data-toggle="modal" data-target="#popuppedido_<?=$ido_DoItem;?>"  aria-expanded="true"><i style="background-color: #34af23;color:#ffffff;padding-left:5px;padding-top: 5px; padding-right: 7px;padding-bottom: 5px; border-radius: 4px;" class="fa fa-cart-plus" aria-hidden="true"></i></a>
														<?php
													endif;
													?>
													<!-- Modal -->
																		...
																		for ($i=0; $i < $totlaraytamanhos; $i++) { 
																			...
																			if($lerbanco->getResult()):
																				foreach ($lerbanco->getResult() as $iptiosnarray):
																					...
																					$valordoption = ($meio_a_meio == "true" ? $preco_item : $valor_tamanho);
																					echo "<label>
																					<input type=\"radio\" value=\"{$nome_option}-_-{$valordoption}\" data-iddacategoria=\"{$iddacategoria}\" data-iddoitem=\"{$ido_DoItem}\" data-idtaamanho=\"{$id_option}\" class=\"tamanho_item\"  name=\"valor\" />{$nome_option} <span> R$ ".Check::real($valordoption)."</span>
																					</label>
																					";
																				endforeach; 
																			endif;										
...
if($lerbanco->getResult()):
	echo "<h5 class='hcinco'>Escolha + <b id='valortotalint_{$ido_DoItem}'></b> sabor!</h5>";
	foreach ($lerbanco->getResult() as $adicionalarray):
		extract($adicionalarray);
		$checkopsabor = ($id == $ido_DoItem ? "checked disabled" : "");
		echo "<label>
		<input type=\"checkbox\" {$checkopsabor} class=\"controlecheckbox\" name=\"sabormeioameio[]\" value=\"{$nome_item}-_-{$preco_item}\" /> {$nome_item} 
		<span>".($preco_item == '0.00' ? '' : 'R$ '.Check::real($preco_item))."</span>
		</label>";
	endforeach;
	echo "<br /><small style=\"color: red;\">* Será cobrado pelo sabor de maior valor</small>";
endif;
...
<!--FIM DA QUANTIDADE-->
<br />
<input type="text" class="form-control obsitem" value="" placeholder="<?=$texto['msg_obsItem'];?>" name="observacao">
...
if(!empty($disponivel) && $disponivel == 1):
	?>	
	<button style="cursor: pointer;" class="adicionarItemPedido_<?=$ido_DoItem;?>" id="btn-adicionar-item"><?=$texto['msg_purchase'];?></button>
	<script type="text/javascript">
		$(document).ready(function(){

			$('.adicionarItemPedido_<?=$ido_DoItem;?>').click(function(){
				$('.adicionarItemPedido_<?=$ido_DoItem;?>').html('AGUARDE...');
				$('.adicionarItemPedido_<?=$ido_DoItem;?>').prop('disabled', true);
				...
				$.ajax({
					...
					success: function(data){				
						...
						$('.adicionarItemPedido_<?=$ido_DoItem;?>').html('<?=$texto['msg_purchase'];?>');
						$('.adicionarItemPedido_<?=$ido_DoItem;?>').prop('disabled', false);						
						$('.qtdpedido').val('1');
					}
				});
			...
			$.ajax({
				...
				success:function(data){
					...
					if(data > 0){
						...
						$('#cf_group_meioameio'+iddoitem+' input[type=checkbox]').change(function(){
